...
const convertStreamToJSON = (outData) => {
    ...
    logger.info(`Socket ${result.socket}: ${result.chartData.labels.length} from overall ${outData.length} data points were retrieved for chart (factor ${dataFactor})`);
    ...
    return renderChart('line', options, dataObject.chartData).then(chartDiv => {
        ...
        return promisify(fs.readFile)(path.join(__dirname, templateFile)).then(res => {
            ...
            return promisify(fs.writeFile)(outputFile, html).then(res => {
                logger.info(`The chart was rendered in ${outputFile}!`);
            }).catch(err => {
                logger.error(err);
            });
        });
...
/** calculates the overall used energy by @param calculation */
const calculateUsedEnergy = (calculation, measurements) => {
    return calculationHelper.calculateOverallUsedEnergy(calculation, measurements, calculateCO2Emission);
};

/** calculates co2 emission in kg for given @param usedEnergy in kWh 
 * source: https://www.umweltbundesamt.de/sites/default/files/medien/1410/publikationen/2017-05-22_climate-change_15-2017_strommix.pdf
*/
const calculateCO2Emission = usedEnergy => {
    return Math.round((usedEnergy * 0.527) * 100) / 100; // 2 digits
};
